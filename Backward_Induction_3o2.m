...
V_c_benchmark=[13.9225;7.2047;2.7347];


%% grid of state variable
L=38;
n_grid=200;
n_l=n_grid-160;n_r=n_grid;% asymmetric truncation boundary
delta_omega=L/n_r;
xi_m=-2.4;xi_p=-1.9;% dampening factor
...
a_v_ini=mean_var-30/(1+q_CIR);    % a proper initial guess for interval boundary
b_v_ini=mean_var+27/(1+q_CIR);
sigma_ini=log(v_ini);
...
n_v=27;                         % total nodes for log-variance. When N=252, J=155 is enough.
...
% k_i=l_x*(p*eta_x1/(eta_x1-1)+(1-p)*eta_x2/(eta_x2+1)-1); % double exponential jumps
...
% hybrid method for the modified Bessel function of the first kind with complex order
Bessel_threshold=40; 
