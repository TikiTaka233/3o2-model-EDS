...
r=0.01;q=0.02;sigma=0.1;mu_hat=-0.5;k_hat=0.01;lambda=1;v_bar=1.0;eta=1;rho=0;v_0=1; %  NIG+CIR v_bar=0.7, 1.0, 1.1
K=100;
Tmat=1;
N_monitor=12;
delta_t=Tmat/N_monitor;

% r=0.05;q=0;C=4;G=50;M=60;Y=0.7;lambda=0.602;v_bar=1.3;eta=0.9;rho=0;v_0=1; %  CGMY+CIR v_bar=0.7, 1.0, 1.1
% K=100;% strike price
% Tmat=1;N_monitor=25;delta_t=Tmat/N_monitor;

%% grid of state variable
L=78; 
% truncation range NIG-CIR model: L=78 for N=12, L=86 for N=25, L=77 for N=50; 
% CGMY-CIR model: L=45 for N=12, L=47 for N=25, L=47 for N=50
n_r=490;n_l=60; 
% truncation level NIG-CIR model: n_r=490, n_l=60 for N=12, n_r=540, n_l=60 for N=25, n_r=1020, n_l=130 for N=50; 
% CGMY-CIR model: n_r=300, n_l=60 for N=12, n_r=300, n_l=70 for N=25, n_r=580, n_l=130 for N=50
delta_omega=L/n_r;
...
n_v=53;                         % total nodes for log-variance. NIG-CIR model: J=53 for N=12, J=85 for N=25, J=115 for N=50; CGMY-CIR model: J=67 for N=12, J=100 for N=25, J=144 for N=50
...
%% NIG+CIR
Chara_m=zeros(n_l+n_r+1,n_v,n_v);Chara_p=zeros(n_l+n_r+1,n_v,n_v);
psi_m=sqrt(lambda^2+2*eta^2/k_hat*(sqrt(1+k_hat*sigma^2*omegam1.^2+2*1i*mu_hat*k_hat*omegam1)-1+1i*omegam1*(sqrt(1-k_hat*sigma^2-2*mu_hat*k_hat)-1)));
psi_p=sqrt(lambda^2+2*eta^2/k_hat*(sqrt(1+k_hat*sigma^2*omegap1.^2+2*1i*mu_hat*k_hat*omegap1)-1+1i*omegap1*(sqrt(1-k_hat*sigma^2-2*mu_hat*k_hat)-1)));
...
Bessel_chf_2_0=exp((Zeta'+Zeta)/2);Bessel_chf_2_0=reshape(Bessel_chf_2_0,1,n_v,n_v);
...
Chara_2=exp(Zeta').*exp(q_CIR/2*(Zeta'-(Zeta-lambda*delta_t)));Chara_2=reshape(Chara_2,1,n_v,n_v);
Exp1m=-1i*omegam1*(r-q)*delta_t;Exp1p=-1i*omegap1*(r-q)*delta_t;
Exp2m=(1i*omegam1*rho*eta+lambda)/eta^2;Exp2p=(1i*omegap1*rho*eta+lambda)/eta^2;
Exp3m=psi_m.*(1+exp(-psi_m*delta_t))./((1-exp(-psi_m*delta_t))*eta^2);Exp3p=psi_p.*(1+exp(-psi_p*delta_t))./((1-exp(-psi_p*delta_t))*eta^2);
Zeta_m=exp(Zeta')-exp(Zeta);Zeta_m=reshape(Zeta_m,1,n_v,n_v);
Zeta_p=exp(Zeta')+exp(Zeta);Zeta_p=reshape(Zeta_p,1,n_v,n_v);

Bessel_chf_m=Bessel_chf_m_1.*Bessel_chf_2_0;
Bessel_chf_p=Bessel_chf_p_1.*Bessel_chf_2_0;
...
Chara_2=exp(Zeta).*exp(q_CIR/2*(Zeta-(log(v_0)-lambda*delta_t)));

Bessel_chf_m_0=Bessel_chf_m_1*Bessel_chf_2_0;
Charavm01=(Chara_m_1*Chara_2.*exp(Exp1m-Exp2m*(exp(Zeta)-v_0)-Exp3m*(exp(Zeta)+v_0)+abs(real(Bessel_chf_m_0))).*besseli(q_CIR,Bessel_chf_m_0,1));

Bessel_chf_p_0=Bessel_chf_p_1*Bessel_chf_2_0;
Charavp01=(Chara_p_1*Chara_2.*exp(Exp1p-Exp2p*(exp(Zeta)-v_0)-Exp3p*(exp(Zeta)+v_0)+abs(real(Bessel_chf_p_0))).*besseli(q_CIR,Bessel_chf_p_0,1));
%% CGMY+CIR
% Chara_m=zeros(n_l+n_r+1,n_v,n_v);Chara_p=zeros(n_l+n_r+1,n_v,n_v);
% psi_m=sqrt(lambda^2+2*eta^2*C*gamma(-Y)*((M^Y-(M+1i*omegam1).^Y+G^Y-(G-1i*omegam1).^Y)+1i*omegam1*(M^Y-(M-1)^Y+G^Y-(G+1)^Y)));
% psi_p=sqrt(lambda^2+2*eta^2*C*gamma(-Y)*((M^Y-(M+1i*omegap1).^Y+G^Y-(G-1i*omegap1).^Y)+1i*omegap1*(M^Y-(M-1)^Y+G^Y-(G+1)^Y)));
% Bessel_chf_m_1=4*psi_m.*exp(-1/2*psi_m*delta_t)./(eta^2*(1-exp(-psi_m*delta_t)));
% Bessel_chf_p_1=4*psi_p.*exp(-1/2*psi_p*delta_t)./(eta^2*(1-exp(-psi_p*delta_t)));
% Bessel_chf_2=exp((Zeta'+Zeta)/2);Bessel_chf_2=reshape(Bessel_chf_2,1,n_v,n_v);
% Chara_m_1=2*psi_m.*exp(-1/2*(psi_m-lambda)*delta_t)./(eta^2*(1-exp(-psi_m*delta_t)));
% Chara_p_1=2*psi_p.*exp(-1/2*(psi_p-lambda)*delta_t)./(eta^2*(1-exp(-psi_p*delta_t)));
% Chara_2=exp(Zeta').*exp(q_CIR/2*(Zeta'-(Zeta-lambda*delta_t)));Chara_2=reshape(Chara_2,1,n_v,n_v);
% Exp1m=-1i*omegam1*(r-q)*delta_t;Exp1p=-1i*omegap1*(r-q)*delta_t;
...
V_c_m=(V_p_m+exp(-r*Tmat)*(S_0/(N_monitor+1)*(1-exp((r-q)*(N_monitor+1)*delta_t))/(1-exp((r-q)*delta_t))-K)); % option price with damping factor alpha_1
...
V_c_p=(V_p_p+exp(-r*Tmat)*(S_0/(N_monitor+1)*(1-exp((r-q)*(N_monitor+1)*delta_t))/(1-exp((r-q)*delta_t))-K)); % option price with damping factor alpha_2
